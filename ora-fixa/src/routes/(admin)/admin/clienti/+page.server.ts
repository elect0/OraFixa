...
export const load: PageServerLoad = async ({ locals: { supabase, session }, url }) => {
	...
	const [{ data: clients }, { data: count }, { data: services }] = await Promise.all([
		clientsPromise,
		countPromise,
		servicesPromise
	]);
...
export const actions = {
	...
	blockClient: async ({ request, locals: { supabase } }) => {
		const form = await request.formData();
		const userId = form.get('clientId') as string;

		const { error } = await supabase.from('profiles').update({is_banned: true}).eq('id', userId);

		if (error) {
			console.error(error);
			return fail(400, { message: 'Eroare la banarea clientului.' });
		}
